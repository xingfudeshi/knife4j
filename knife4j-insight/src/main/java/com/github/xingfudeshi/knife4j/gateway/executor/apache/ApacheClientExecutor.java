...
package com.github.xingfudeshi.knife4j.gateway.executor.apache;

import cn.hutool.core.collection.CollectionUtil;
import com.github.xingfudeshi.knife4j.gateway.executor.GatewayClientExecutor;
import com.github.xingfudeshi.knife4j.gateway.context.GatewayRequestContext;
import com.github.xingfudeshi.knife4j.gateway.executor.apache.pool.PoolingConnectionManager;
import com.github.xingfudeshi.knife4j.gateway.executor.response.GatewayClientResponse;
import com.github.xingfudeshi.knife4j.common.model.HeaderWrapper;
import com.github.xingfudeshi.knife4j.gateway.executor.response.ApacheClientResponse;
import com.github.xingfudeshi.knife4j.gateway.executor.response.DefaultClientResponse;
 ...
 * 2020/10/29 20:35
 */
public class ApacheClientExecutor extends PoolingConnectionManager implements GatewayClientExecutor {
    
    Logger logger = LoggerFactory.getLogger(ApacheClientExecutor.class);
    
    private HttpUriRequest buildRequest(GatewayRequestContext routeContext) {
    ...
    public GatewayClientResponse executor(GatewayRequestContext routeContext) {
        GatewayClientResponse gatewayClientResponse = null;
        try {
            // 判断当前接口是否需要执行basic
            CloseableHttpResponse closeableHttpResponse = getClient().execute(buildRequest(routeContext));
            gatewayClientResponse = new ApacheClientResponse(closeableHttpResponse);
        } catch (Exception e) {
            logger.error("Executor Failed,message:" + e.getMessage(), e);
            // 当前异常有可能是服务下线导致
            if (e instanceof HttpHostConnectException) {
                // 服务下线，连接失败
                gatewayClientResponse = new DefaultClientResponse(routeContext.getOriginalUri(), e.getMessage(), 504);
            } else {
                gatewayClientResponse = new DefaultClientResponse(routeContext.getOriginalUri(), e.getMessage());
            }
        }
